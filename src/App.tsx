...
export default function FlashcardApp() {
  ...
  const handleAnswerSubmit = () => {
    if (selectedAnswers.length > 0 && !answerSubmitted) {
      ...
      const correctAnswerArray = flashcards[currentCardIndex].correctAnswer.toLowerCase().split(',').map(a => a.trim())
        ...
        <div key={index} className="mb-2">
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              disabled={answerSubmitted || (selectedAnswers.length >= requiredCount && !selectedAnswers.includes(letter))}
            />
        ...
        {flashcards.length > 0 ? (
          <div className="mt-6">
            <div className="flashcard bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl p-6 w-full">
              ...
              <h3 className="text-xl font-semibold mb-3 text-pink-300">Possible Answers:</h3>
              ...
              {showAnswer && (
                <div className="mt-6 bg-purple-700 bg-opacity-50 p-4 rounded-lg">
                  ...
                  <p className="text-white text-lg">
                    {flashcards[currentCardIndex].correctAnswer.split(',').map(answer => 
                      String.fromCharCode(65 + answer.toLowerCase().charCodeAt(0) - 97)
                    ).join(', ')}
                  </p>
                  {answerSubmitted && (
                    <div className="mt-2 flex items-center">
                      {selectedAnswers.join(',').toLowerCase() === flashcards[currentCardIndex].correctAnswer.toLowerCase() ? (
                        <CheckIcon className="text-green-500 mr-2" />
                      ) : (
                        <XIcon className="text-red-500 mr-2" />
                      )}
                      <span className="text-white">
                        {selectedAnswers.join(',').toLowerCase() === flashcards[currentCardIndex].correctAnswer.toLowerCase() ? "Correct!" : "Incorrect"}
                      </span>
                    </div>
                  )}
                </div>
